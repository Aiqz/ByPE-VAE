...
class VAE(AbsModel):
    ...
    def create_model(self, args, train_data_size=None):
        ...
        self.q_z_layers = nn.Sequential(
            ...
            # nn.ELU(),
            weight_norm(
                nn.Conv2d(in_channels=self.cs * 2, out_channels=self.bottleneck, kernel_size=3, stride=1, padding=1)),
            nn.ELU(),
        )
        h_size = 1536
        # self.q_z_mean = weight_norm(nn.Conv2d(in_channels=self.cs*2, out_channels=self.bottleneck, kernel_size=3, stride=1, padding=1))
        self.q_z_mean = weight_norm(nn.Linear(h_size, self.args.z1_size))
        ...
        self.q_z_logvar = weight_norm(nn.Linear(h_size, self.args.z1_size))
        self.p_x_layers_pre = nn.Sequential(weight_norm(nn.Linear(self.args.z1_size, h_size)))
        self.p_x_layers = nn.Sequential(
            # weight_norm(nn.Linear(self.args.z1_size, h_size)),
