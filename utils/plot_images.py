...
def generate_fancy_grid(config, dir, reference_data, generated, col_num=4, row_num=3):
    ...
    for k in range(len(reference_data)):
        ...
        # plt.savefig(generated_dir + "generated.png")
        # plt.imsave(arr=np.transpose(np.squeeze(original_image), (1,2,0)), fname='test_{}.png'.format(k))
        plt.imsave(arr=grid, fname=generated_dir + "generated_{}.png".format(k))
...
def plot_images(config, x_sample, dir, file_name, size_x=3, size_y=3):
    ...
    plt.savefig(dir + file_name + '.pdf', bbox_inches='tight')
    plt.close(fig)
